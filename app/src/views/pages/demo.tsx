...
function pageHref(page: number): string {
  return page <= 1 ? route.demo() : `${route.demo()}?page=${page}`
}
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-zinc-50 to-teal-50/30">
      <div className="mx-auto max-w-3xl px-6 py-14">
        <a
          href={route.home()}
          className="text-sm font-medium tracking-wide text-teal-700 hover:text-teal-900"
        >
          ← Namix
        </a>
        ...
        <ol className="mt-10 divide-y divide-zinc-200 border-y border-zinc-200">
          {items.map((item) => (
            <li key={item.id} className="flex gap-4 py-4">
              <span className="w-12 shrink-0 text-sm font-medium text-teal-700">{item.id}</span>
              <div>
        ...
        <nav className="mt-8 flex flex-wrap items-center justify-between gap-3 text-sm" aria-label="分页">
          {page > 1 ? (
            <a
              ...
              className="rounded-lg border border-zinc-300 bg-white px-3 py-1.5 text-zinc-700 hover:border-teal-600 hover:text-teal-800"
            >
              ← 上一页
            </a>
          ) : (
          ...
          <div className="flex flex-wrap gap-1">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <a
                ...
                className={
                  p === page
                    ? 'rounded-lg bg-teal-700 px-2.5 py-1.5 font-medium text-white'
                    : 'rounded-lg px-2.5 py-1.5 text-zinc-600 hover:bg-white hover:text-teal-800'
                }
